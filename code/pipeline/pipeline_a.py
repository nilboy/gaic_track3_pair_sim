...
processes = [
    subprocess.Popen(
        f"python model_processor/build_ensemble_model_a.py", shell=True
    )
]
for process in processes:
    process.wait()
...
cmd = f"export CUDA_VISIBLE_DEVICES=0,1,2,3 && python train/train_classification_a.py --max_run_time={72 - timer.get_current_time()}"
logger.info(cmd)
os.system(cmd)
