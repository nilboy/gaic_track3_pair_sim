...
def train_model(model_name):
    ...
    run_gpu_model(f'bash train/train_mlm.sh {model_name}', log_file=f"{model_name}_mlm.log", max_time=170)
    logger.info(f"预训练 {model_name} 结束: {timer.get_current_time()}")

# 预训练
model_processes = []
from multiprocessing import Process
for model_name in ['bert-large', 'macbert-large', 'roberta-large',
                   'bert-base-fgm']:
...
model_processes = []
from multiprocessing import Process
for model_name in ['macbert-base-fgm', 'roberta-base-fgm',
                   'bert-base', 'macbert-base', 'roberta-base', 'bert-base2', 'macbert-base2']:
    p = Process(target=train_model, args=(model_name,))
    p.start()
    time.sleep(60)
    logger.info(f'创建模型训练 {model_name}...')
...
def train_classification(model_name):
    # 预训练
    logger.info(f"训练分类器 {model_name} 开始: {timer.get_current_time()}")
    run_gpu_model(f'python train/train_single_model.py --model_name={model_name}', log_file=f"{model_name}.log", max_time=80)
    logger.info(f"训练分类器 {model_name} 结束: {timer.get_current_time()}")

logger.info(f'训练分类模型   : {timer.get_current_time()} 小时')
model_processes = []
from multiprocessing import Process
for model_name in ["bert-large", "macbert-large", "roberta-large",
                   "bert-base", "macbert-base", "roberta-base",
                   "macbert-base_fgm", "bert-base_fgm", "roberta-base_fgm", "bert-base2", "macbert-base2"]:
    p = Process(target=train_classification, args=(model_name,))
    p.start()
    time.sleep(30)
    logger.info(f'创建模型训练 {model_name}...')
    model_processes.append(p)
