...
def get_all_files_from_model_name(task):
    input_data_dir = f'../user_data/soft_labels/{task}'
    file_name_list = []
    for file_name in os.listdir(input_data_dir):
        logger.info(f"collect file: {file_name}")
...
def construct_model_data(train_rate: float=0.55,
    ...
    file_name_list = get_all_files_from_model_name(task)
    merge_results = {}
    ...
    if task == 'classification':
        ...
        convert_single_file("../user_data/data/train_data/train.jsonl",
                            "../user_data/data/train_data/train_regression_all.jsonl", merge_results, train_rate)
