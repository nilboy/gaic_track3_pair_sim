...
def get_tensor_info(model_id, tensor_name, tensor_value):
    model_id = str(model_id)
    # classifier
    if tensor_name.startswith('classifier'):
        return f"{model_id}_classifier", tensor_name.replace('.', '/'), tensor_value
    ...
    return model_id, new_tensor_name, new_tensor_value

def get_tf_tensor_dict(model_names):
    output_dict = {}
    for model_id in range(len(model_names)):
        output_dict[str(model_id)] = {}
        output_dict[f'{model_id}_classifier'] = {}

    for model_id, model_name in enumerate(model_names):
        for k, v in torch.load(os.path.join(model_name, 'pytorch_model.bin'),
                               map_location='cpu').items():
            v = v.numpy()
            try:
                param_type, new_name, new_value = get_tensor_info(model_id, k, v)
...
def load_configs(model_names, use_fp16,
    ...
    for model_name in model_names:
        submodel_config = BertConfigBase.from_pretrained(model_name)
...
def load_ensemble_weights(model_names, config_list):
    output_dict = get_tf_tensor_dict(model_names)
    output_weights_dict = {}
    for model_id in range(len(config_list)):
        ...
        output_weights_dict[f'{model_id}_classifier'] = get_classifier_weights(output_dict[f'{model_id}_classifier'])
...
def ensemble_score_output(score_list, network):
    output_score_list = []
    for score_item in score_list:
        OUT = network.add_shuffle(score_item)
        OUT.reshape_dims = (1, 2, 1)
        output_score_list.append(OUT.get_output(0))
    output_score_layer = network.add_concatenation(output_score_list)
    output_score_list = output_score_layer.get_output(0)
    output_score_out = network.add_reduce(output_score_list,
                                          op=trt.ReduceOperation.AVG,
                                          axes=1 << 0,
                                          keep_dims=False).get_output(0)
    return output_score_out
...
def build_engine(batch_sizes, workspace_size, sequence_length, config_list, weights_dict, squad_json, vocab_file,
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(
        ...
        for model_id in range(len(config_list)):
            ...
            output_score = classifier_output(pooled_output, network, weights_dict[f'{model_id}_classifier'])
            output_score_list.append(output_score.get_output(0))
        output_score_out = ensemble_score_output(output_score_list, network)
...
def main():
    ...
    # 获取所有模型名称
    model_names = [os.path.join(config_path, model_name,
                                'outputs') for model_name in os.listdir(config_path) if os.path.isdir(os.path.join(config_path, model_name))]

    config_list = load_configs(model_names, args.fp16, args.int8, args.strict, args.force_fc2_gemm,
                        args.force_int8_skipln, args.force_int8_multihead, args.int8 and args.onnx != None)
    weights_dict = load_ensemble_weights(model_names, config_list)
